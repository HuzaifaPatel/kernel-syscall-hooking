...
static int vmx_handle_exit(struct kvm_vcpu *vcpu, fastpath_t exit_fastpath)
{
	...
	// HUZI EDIT STARTS
	msr_get_info.data = 0x00;
	msr_get_info.index = 0xC0000080;
	msr_get_info.host_initiated = true;

	if (!vmx_get_msr(vcpu, &msr_get_info)){
		if(msr_get_info.data == 3328){
			printk(KERN_INFO "THE RETURN VALUE IS: 0 and the msr_get_info.data is %llu \n", msr_get_info.data);
			msr_get_info.data = 3329;
			vmx_set_efer(vcpu, msr_get_info.data);

			// vmx_set_msr(vcpu, &msr_get_info);
		}else{
			printk(KERN_INFO "ELSE STATEMENT: the msr_get_info.data is %llu \n", msr_get_info.data);
			//msr_get_info.data = msr_get_info.data || 0x00000001;
		
			//vmx_set_msr(vcpu, &msr_get_info);
		}
	}else{
		printk(KERN_INFO "THE RETURN VALUE IS: 1 and the msr_get_info.data is NULL \n");
	}
